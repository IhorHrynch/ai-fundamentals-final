...
def visualize_difference(input_feature: float, prediction: ArrayLike):
    ...
    difference = abs(actual_target - prediction)

    # Visualization
    fig = plt.figure(figsize=(6,4))
    # Plot entire dataset
    plt.scatter(X, y, color='grey', alpha=0.6, label='Dataset')

    # Plot actual target
    plt.scatter(input_feature, actual_target, color='blue', s=100, label='Actual Target')

    # Plot predicted target
    plt.scatter(input_feature, prediction, color='red', s=100, label='Predicted Target')

    # Draw dashed line between actual and predicted
    plt.plot(
